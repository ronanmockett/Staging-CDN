<?php if ($this->status === 'failed') {
    echo "<div><p class='stgcdn_notice error'><strong>" . printf( __('%s', 'stgcdn'), $this->status ) . ":</strong> " . printf( __('%s', 'stgcdn'), $this->error ) . "</p></div>";
} elseif ($this->status === 'success') {
    echo "<div><p class='stgcdn_notice success'><strong>" . printf( __('%s', 'stgcdn'), $this->status ) . ":</strong> " . __('Your settings have been saved.', 'stgcdn') . "</p></div>";
} ?>
    ...
    <div class="panel">
        ...
        <form action="?page=stgcdn-admin" method="post">
            <div class="options_wrapper">
                ...
                <label><?php _e('Your Staging URL', 'stgcdn'); ?></label>
